...
def train_rnn(fold_i,
    ...
    classifier = RNN(rng, shape = (img_h, img_w), hidden_size,cell, batch_size =50, p =0.5)
    ...
    test_error = T.mean(T.neq(test_y_pred, y))##Returns a variable representing the result of logical inequality (a!=b)
        ...
        if True: # legacy conditional
            ...
            for i in range(len(datasets[1])):
                ...
                with open("/mnt/test_predictions/test_predictions_gpu{}_b{}_e{}".format(gpuno,i,epoch), "w") as outf:
                    pickle.dump({"predictions":test_predictions_i, "tracker": test_tracker_i}, outf)
            print(mode)
            if mode == "em":
                for i in range(len(train_batches)):
                    ...
                    with open("/mnt/train_predictions/train_predictions_gpu{}_b{}_e{}".format(gpuno,i,epoch), "w") as outf:
                        pickle.dump({"predictions":train_predictions_i, "tracker": train_tracker_i}, outf)
...
def safe_update(dict_to, dict_from):
    ...
    """
    for key, val in list(dict(dict_from).items()):
        if key in dict_to:
            raise KeyError(key)
        dict_to[key] = val
    return dict_to
...
def run():
    ...
    if mode == "EM":
        ...
        init_preds, eid2rows = init_curr_preds(len(sents_train), E, Epos)
    s = 9
    perf = train_rnn(i, datasets,trackers,
                          U,
                          lr_decay=0.95,
                          filter_hs=[3,4,5],
                          conv_non_linear="relu",
